 ...
 */
fn se_get_reference(card: &mut sc_card, file_id: i32, se_reference: u8, search_template: &sc_crt) -> c_ulong
{
    let mut result = SC_AC_KEY_REF_NONE;
    let dp = unsafe { Box::from_raw(card.drv_data as *mut DataPrivate) };
    let file_id = u16::try_from(file_id).unwrap();
...
//println!("parsed: {:X?}", tlv);
        ...
        assert_eq!(usize::try_from((tag & 0x0F).count_ones()).unwrap(), tlv.length());
...
mod tests {
    ...
    fn test_se_parse_sac() {
        ...
        let rv = se_parse_sac(/*card: &mut sc_card,*/ 1, &data, &mut sac_info);
...
//        assert_eq!(sac_info.next, std::ptr::null_mut());
        ...
        assert_eq!(sac_info.crts[0],   sc_crt{tag: 0xA4, usage: 8, algo: 0, refs: [0x81,0,0,0,0,0,0,0]});
        ...
        let rv = se_parse_sac(/*card: &mut sc_card,*/ 2, &data, &mut sac_info);
        assert_eq!(usize::try_from(rv).unwrap(), data.len()-3);
        assert_eq!(sac_info.reference, 2);
        assert_eq!(sac_info.crts_len,  3);
        assert_eq!(sac_info.crts[0],   sc_crt{tag: 0xB4, usage: 8, algo: 2, refs: [   1,0,0,0,0,0,0,0]});
        assert_eq!(sac_info.crts[1],   sc_crt{tag: 0xB8, usage: 8, algo: 2, refs: [   1,0,0,0,0,0,0,0]});
        assert_eq!(sac_info.crts[2],   sc_crt{tag: 0xA4, usage: 8, algo: 0, refs: [0x81,0,0,0,0,0,0,0]});

        sac_info =  SACinfo::default();
        let data : [u8; 14] = [/* 80 01 04 */
            0xA4, 0x09, 0x83, 0x01, 0x01, 0x83, 0x01, 0x81, 0x95, 0x01, 0x08,      0x00, 0x00, 0x00];
        let rv = se_parse_sac(/*card: &mut sc_card,*/ 4, &data, &mut sac_info);
        assert_eq!(usize::try_from(rv).unwrap(), data.len()-3);
        assert_eq!(sac_info.reference, 4);
        assert_eq!(sac_info.crts_len,  1);
        assert_eq!(sac_info.crts[0],   sc_crt{tag: 0xA4, usage: 8, algo: 0, refs: [0x01,0x81,0,0,0,0,0,0]});
        ...
        let rv = se_parse_sac(/*card: &mut sc_card,*/ 5, &data, &mut sac_info);
        assert_eq!(usize::try_from(rv).unwrap(), data.len()-3);
        assert_eq!(sac_info.reference, 5);
        assert_eq!(sac_info.crts_len,  3);
        assert_eq!(sac_info.crts[0],   sc_crt{tag: 0xB4, usage: 0x30, algo: 2, refs: [0x84,0,0,0,0,0,0,0]});
        assert_eq!(sac_info.crts[1],   sc_crt{tag: 0xB8, usage: 0x30, algo: 2, refs: [0x84,0,0,0,0,0,0,0]});
        assert_eq!(sac_info.crts[2],   sc_crt{tag: 0xA4, usage: 0x80, algo: 0, refs: [0x82,0,0,0,0,0,0,0]});

        sac_info =  SACinfo::default();
        let data : [u8; 9] = [/* 80 01 06 */
            0x00, 0x00, 0x00, 0x00, 0x00, 0x00,     0x00, 0x00, 0x00];
        let rv = se_parse_sac(/*card: &mut sc_card,*/ 6, &data, &mut sac_info);
        assert_eq!(rv, 0);
        ...
        assert_eq!(sac_info.reference, 1);
        assert_eq!(sac_info.crts_len,  1);
        assert_eq!(sac_info.crts[0],   sc_crt{tag: 0xA4, usage: 8, algo: 0, refs: [1,0,0,0,0,0,0,0]});
    }
/*
    #[test]
